...
class GetBinCyclecountViewSet(StockBinViewSet):
    """
        list:
            Response a data list（get）
    """
    ...
    def list(self, request, *args, **kwargs):
        staff_name = staff.objects.filter(openid=self.request.auth.openid, id=self.request.META.get('HTTP_OPERATOR')).first().staff_name
        queryset = self.filter_queryset(self.get_queryset())
        bin_name = self.request.GET.get('bin_name', '')
        for i in queryset:
            if (d:=ManualCyclecountModeModel.objects.filter(cyclecount_status=0, bin_name=bin_name, goods_code=i.goods_code)).exists():
                d.delete()
            data = {
                'openid': self.request.auth.openid,
                'creater': staff_name,
            ...
            serializer.is_valid(raise_exception=True)
            serializer.save()
        queryset = ManualCyclecountModeModel.objects.filter(bin_name=bin_name, cyclecount_status=0)
        page = self.paginate_queryset(queryset)
        if page is not None:
            serializer = serializers.ManualCyclecountGetSerializer(instance=page, many=True)
            return self.get_paginated_response(serializer.data)
