      ...
      >
         <template v-slot:top>
           <q-btn-group push>
             ...
             <q-btn label="分配拣货员" icon="how_to_vote" @click="allocateStaff()">
               <q-tooltip content-class="bg-amber text-black shadow-4" :offset="[10, 10]" content-style="font-size: 12px">
                 把拣货单分配给拣货员
               </q-tooltip>
             </q-btn>
  ...
  methods: {
    getList () {
      ...
      if (_this.$q.localStorage.has('auth')) {
        getauth(_this.pathname + '?page=' + this.current + '&picking_status=0&order_line=1', {
        }).then(res => {
    ...
    cancelSubmit () {
      ...
      _this.filter_data.bin_data = ''
      _this.filter_data.staff_name = ''
    },
    resSubmit () {
      var _this = this
      if (_this.$q.localStorage.has('auth')) {
        postauth(baseurl + 'dn/onepicking/?page=' + this.current + '&picking_status=0&order_line=1&bin_name__startswith=' + _this.filter_data.bin_data, _this.filter_data).then(res => {
          _this.cancelSubmit()
          _this.getList()
          _this.$q.notify({
            message: '分配成功',
